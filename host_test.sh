...
status=$(curl --retry 2 --retry-delay 3 --retry-all-errors -s -L --output /tmp/title --write-out "%{http_code}" "$h" 2>/dev/null | head -n1 | tr -dc '0-9')
[[ $status -gt 400 ]] && status=$(curl --retry 2 --retry-delay 3 --retry-all-errors -A 'Mozilla/5.0' -s -L --output /tmp/title --write-out "%{http_code}" "$h" 2>/dev/null | head -n1 | tr -dc '0-9')
